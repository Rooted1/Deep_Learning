...
class QLoRALinear(Linear4Bit):
    def __init__(
        ...
        self.lora_a = torch.nn.Linear(in_features, lora_dim, bias=False).to(torch.float32)
        self.lora_b = torch.nn.Linear(lora_dim, out_features, bias=False).to(torch.float32)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (base + lora).to(orig_dtype)
